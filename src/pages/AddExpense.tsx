...
export function AddExpense() {
  ...
  return (
    <motion.div
      ...
      className="flex flex-col max-w-lg mx-auto pb-10 min-w-0"
    >
      {/* COMPACT HEADER */}
      <header className="flex justify-between items-center py-2 min-w-0">
        <button 
          onClick={() => navigate(-1)} 
          className="p-1.5 bg-gray-100 dark:bg-white/10 rounded-full text-gray-600 dark:text-white/70 hover:text-black dark:hover:text-white transition-colors border border-gray-200 dark:border-white/10"
        >
          <X className="w-4 h-4" />
        </button>

        <div className="flex bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-full p-1 shadow-sm">
          <button 
            className={cn(
      ...
      <div className="flex flex-col items-center justify-center py-3 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl p-4 my-2 shadow-sm">
        ...
        <input 
          ...
          className="w-full max-w-xs text-center text-xs bg-gray-50 dark:bg-black/20 border border-gray-200 dark:border-white/10 rounded-xl py-1.5 px-3 outline-none focus:ring-2 focus:ring-blue-500/40 mb-2"
        />

        {/* Date Selector Toggle */}
        <div className="flex items-center gap-2">
          <label className="flex items-center gap-1.5 cursor-pointer bg-gray-100 dark:bg-white/5 px-3 py-1 rounded-full border border-gray-200 dark:border-white/10">
            <input 
              type="checkbox" 
            ...
            {useCustomDate && (
              <motion.div initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: 'auto' }} exit={{ opacity: 0, width: 0 }} className="overflow-hidden">
                <input 
                  ...
                  className="bg-gray-100 dark:bg-white/10 border border-gray-200 dark:border-white/20 rounded-full px-3 py-1 text-[10px] font-semibold text-gray-700 dark:text-white outline-none"
                />
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <div className="flex gap-1.5 overflow-x-auto pb-1 scrollbar-none min-w-0">
            {QUICK_PRESETS.map((p, idx) => (
              <button
                ...
                className="shrink-0 text-[11px] font-bold bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 hover:border-blue-500 text-gray-800 dark:text-gray-200 px-3 py-1.5 rounded-xl transition-all shadow-sm active:scale-95 whitespace-nowrap"
              >
      ...
      <AnimatePresence>
          ...
          >
            <div className="flex justify-between items-center mb-2 px-1">
              ...
              <button 
                ...
                className="text-[10px] font-bold text-blue-500 bg-blue-50 dark:bg-blue-500/20 px-2.5 py-1 rounded-full flex items-center gap-1 hover:scale-105 transition-all"
              >
            ...
            <AnimatePresence>
              {showNewCat && (
                ...
                >
                  <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 p-2.5 rounded-2xl flex gap-2 shadow-sm">
                    <input 
                      type="text" 
                      placeholder="Emoji" 
                      value={newCatEmoji} 
                      onChange={e => setNewCatEmoji(e.target.value)}
                      className="w-12 bg-gray-50 dark:bg-black/20 rounded-xl p-2 text-center text-xs outline-none border border-gray-200 dark:border-white/10"
                    />
                    <input 
                      type="text" 
                      ...
                      className="flex-1 bg-gray-50 dark:bg-black/20 rounded-xl p-2 text-xs outline-none border border-gray-200 dark:border-white/10"
                    />
                    <button 
                      onClick={handleSaveCustomCat}
                      className="bg-blue-500 text-white px-3 rounded-xl text-xs font-bold"
                    >
                      Add
                    </button>
                  </div>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    selectedCatId === cat.id 
                      ? "bg-blue-50 dark:bg-blue-500/20 border-blue-500 text-blue-600 dark:text-blue-400 font-bold shadow-sm scale-105" 
                      : "bg-white dark:bg-white/5 border-gray-200 dark:border-white/10 hover:bg-gray-50 text-gray-700 dark:text-gray-300"
                  )}
                >
            ...
            {categories.length > 8 && (
              <button
                ...
                className="w-full mt-2 py-1.5 rounded-xl bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 text-gray-500 dark:text-gray-400 text-[11px] font-bold flex items-center justify-center gap-1 hover:text-black dark:hover:text-white transition-all"
              >
                {showAllCategories ? (
        ...
        {type === 'income' && (
          ...
          >
            <label className="flex items-center gap-2.5 bg-white dark:bg-white/5 px-4 py-3 rounded-2xl border border-gray-200 dark:border-white/10 shadow-sm cursor-pointer">
              <input 
                type="checkbox" 
                checked={isBorrowed}
      ...
      <button
        ...
        disabled={!isReady}
        className={cn(
          "w-full py-3.5 rounded-2xl flex items-center justify-center gap-2 text-sm font-bold transition-all shadow-md mt-3",
          isReady 
